...
export class Controls extends Scene {
	...
	create() {
		...
		this.title = this.add
			.text(width / 2, height * 0.1, "CONTROLS", {
		...
		this.backBtn = createInteractiveButton(this, "BACK", width / 2, height * 0.9, () => {
	...
	createControls(x: number, y: number, playerLabel: string, playerColor: string, controls: Array<{ key: string; action: string }>) {
		this.add
		...
		controls.forEach((value, index) => {
			...
			this.add.rectangle(x - 140, y + (index + 1) * 100, 60, 60, playerColorHex).setRounded(5);
			this.add
